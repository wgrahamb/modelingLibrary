...
class MockHellfireMassAndMotor:
	...
	def __init__(self):
		...
		self.ISP                = 100 # Seconds. Adjusted to represent a hellfire.
		self.INITIAL_TOTAL_MASS = 45 # Kilograms.
		self.FINAL_TOTAL_MASS   = 25 # Kilograms.
		self.EXIT_VEL           = self.ISP * self.STANDARD_GRAVITY # m/s
		self.DELTA_V            = np.log(self.INITIAL_TOTAL_MASS / \
			self.FINAL_TOTAL_MASS) * self.EXIT_VEL # Meters per second.
		self.MDOT               = (self.INITIAL_TOTAL_MASS - \
			self.FINAL_TOTAL_MASS) / self.BURN_TIME # Kilograms per second.
	...
	def update(self, timeOfFlight, pressure):
		...
		# IF BURNOUT SET FINAL PROPERTIES.
		else:
			if self.FLAG == 0:
				self.FLAG = 1
				self.THRUST = 0.0
