...
int main() 
{
	...
	fstream input("CADAC_SIMS/MAGSIX/input.asc");
	if(input.fail())
	...
	for(int i=0;i<num_vehicles;i++)
	{
		...
		if(strstr(options,"y_plot"))
		{
			if(!strcmp(vehicle_name,"ROTOR"))
			{
				...
				sprintf(index,"%i",i+1);
				plotiasc="CADAC_SIMS/MAGSIX/plot"+string(index)+".asc"; //using Standard Library string constructor
