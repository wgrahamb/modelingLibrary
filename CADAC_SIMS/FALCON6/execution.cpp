...
int main() 
{
	...
	fstream input("CADAC_SIMS/FALCON6/input.asc");
	if(input.fail())
	...
	// cout << "GB right before initialize" << endl; // not here.
	...
	for(int i=0;i<num_vehicles;i++)
	{
		...
		if(strstr(options,"y_plot"))
		{
			if(!strcmp(vehicle_name,"PLANE6"))
			{
				...
				sprintf(index,"%i",i+1);
				plotiasc="CADAC_SIMS/FALCON6/plot"+string(index)+".asc"; //using Standard Library string constructor
	...
	// cout << "GB right after initialize" << endl; // not here.
	...
	// cout << "GB right after screen print" << endl; // not here.

	//Aquire ending time (last entry on 'input.asc')
	end_time=acquire_endtime(input);

	// cout << "GB right after end time acquire" << endl; // not here.
	...
	// cout << "GB right before execute" << endl; // not here.
