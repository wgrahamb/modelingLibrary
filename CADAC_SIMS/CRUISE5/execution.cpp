...
int main(void) 
{
	...
	fstream input("CADAC_SIMS/CRUISE5/input.asc");
	if(input.fail())
	...
	for(int i=0;i<num_vehicles;i++)
	{
		...
		if(strstr(options,"y_plot"))
		{
			if(!strcmp(vehicle_name,"CRUISE3"))
			{
				...
				sprintf(index,"%i",i+1);
				plotiasc="CADAC_SIMS/CRUISE5/plot"+string(index)+".asc"; //using Standard Library string constructor
