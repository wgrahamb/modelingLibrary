...
void guidance(Missile &missile)
{
	...
	magnitude(missile.FLUMissileToPipPosition, forwardLeftUpMissileToInterceptPositionMagnitude);
	...
	else
	{
		...
		vectorProjection(lineOfAttack, missile.fluVel, forwardLeftUpMissileToInterceptLineOfAttackVel);
		double G = 1 - exp(-0.001 * forwardLeftUpMissileToInterceptPositionMagnitude);
		missile.normalGuidanceCommand = 1.5 * (forwardLeftUpMissileToInterceptLineOfSightVel[2] + G * forwardLeftUpMissileToInterceptLineOfAttackVel[2]);
		missile.sideGuidanceCommand = 1.5 * (forwardLeftUpMissileToInterceptLineOfSightVel[1] + G * forwardLeftUpMissileToInterceptLineOfAttackVel[1]);
	}
...
void fly(Missile &missile, bool LogData, bool ConsoleReport, string identity)
{
	...
	if (LogData)
	{
		string fileName = "3DOFS/CPP_3DOF_LINE_OF_ATTACK/output/flyout_" + identity + ".txt";
