...
# Functions.
def WRITE_HEADER(STATE, LOGFILE):
	DATAPOINT = ""
...
def rocketPropulsionFinal(DICT):

    # Handle input.
    CHECKS_AND_BALANCES  = DICT["CHECKS_AND_BALANCES"]
    BORE_RADIUS          = DICT["BORE_RADIUS"]
    END_RADIUS           = DICT["END_RADIUS"]
    INITIAL_GRAIN_LENGTH = DICT["INITIAL_GRAIN_LENGTH"]
    PLOT_FLAG            = DICT["PLOT_FLAG"]
    PLOT2_FLAG           = DICT["PLOT2_FLAG"]
    RPT                  = DICT["RPT"]
    THROAT_AREA_0        = DICT["THROAT_AREA_0"]
    THROAT_DIAM_0        = DICT["THROAT_DIAM_0"]
    EPSILON_0            = DICT["EPSILON_0"]
    NUMBER_OF_GRAINS     = DICT["NUMBER_OF_GRAINS"]
    GRAIN_TEMP           = DICT["GRAIN_TEMP"]
    BALLAST_MASS         = DICT["BALLAST_MASS"]
    ID                   = DICT["ID"]
...
if __name__ == "__main__":
    ...
    BORE_RADII    = np.linspace(0.9, 1.1, 10)
    END_RADII     = np.linspace(2.0, 2.2, 10)
    GRAIN_LENGTHS = np.linspace(3.0, 3.5, 50)
    THROAT_AREAS  = np.linspace(0.65, 1.0, 20)
    EPSILONS      = np.linspace(2.0, 2.3, 20)
    BALLASTS      = np.linspace(0.0, 1.0, 10)
    ...
    THIS_DICT["CHECKS_AND_BALANCES"]  = True
    THIS_DICT["PLOT_FLAG"]            = False
    THIS_DICT["PLOT2_FLAG"]           = False
    THIS_DICT["RPT"]                  = False
    THIS_DICT["GRAIN_TEMP"]           = 70.0

    for I, R in enumerate(RUNS):
        print(I)
        ANOTHER_DICT                         = copy.deepcopy(THIS_DICT)
        ANOTHER_DICT["ID"]                   = str(R)
        ANOTHER_DICT["BORE_RADIUS"]          = R[0]
        ANOTHER_DICT["END_RADIUS"]           = R[1]
        ANOTHER_DICT["INITIAL_GRAIN_LENGTH"] = R[2]
        ANOTHER_DICT["THROAT_AREA_0"]        = R[3]
        ANOTHER_DICT["THROAT_DIAM_0"]        = np.sqrt(ANOTHER_DICT["THROAT_AREA_0"] * 4.0 / np.pi)
        ANOTHER_DICT["EPSILON_0"]            = R[4]
        ANOTHER_DICT["BALLAST_MASS"]         = R[5]
        MAX_HGTS                             = []
        NO_OF_GRAINS                         = [int(2), int(3), int(4)]
        for J, N in enumerate(NO_OF_GRAINS):
            YET_ANOTHER_DICT                     = copy.deepcopy(ANOTHER_DICT)
            YET_ANOTHER_DICT["NUMBER_OF_GRAINS"] = N
            try:
                THIS_RUN                             = rocketPropulsionFinal(YET_ANOTHER_DICT)
                MAX_HGT                              = THIS_RUN[1]
                MAX_HGTS.append(MAX_HGT)
            except:
                print("BUSTED RUN")
        try:
            TEST1 = None
            TEST2 = None
            TEST3 = None
            if MAX_HGTS[0] > 4900 and MAX_HGTS[0] < 5100:
                TEST1 = True
            else:
                TEST1 = False
            if MAX_HGTS[1] > 9900 and MAX_HGTS[1] < 10100:
                TEST2 = True
            else:
                TEST2 = False
            if MAX_HGTS[2] > 14900 and MAX_HGTS[2] < 15100:
                TEST3 = True
            else:
                TEST3 = False
            SUCCESS = False
            if TEST1 == True and TEST2 == True and TEST3 == True:
                SUCCESS = True
            if SUCCESS:
                LOG_FILE = open(f"pythonFunctions/output/{ANOTHER_DICT['ID']}.txt", "w")
                WRITE_HEADER(ANOTHER_DICT, LOG_FILE)
                WRITE_DATA(ANOTHER_DICT, LOG_FILE)
                LOG_FILE.close()
                break
        except:
            print("BUSTED RUN")
